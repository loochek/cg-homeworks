...
void SimpleCompute::SetupSimplePipeline()
{
  ...
  vk_utils::allocateAndBindWithPadding(m_device, m_physicalDevice, { m_source, m_sum}, 0);
...
void SimpleCompute::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkPipeline, unsigned int workgroup_count)
{
  ...
  vkCmdDispatch(a_cmdBuff, workgroup_count, 1, 1);
...
void SimpleCompute::Prepare(unsigned int workgroup_count)
{
  SetupSimplePipeline();
  CreateComputePipeline();

  BuildCommandBufferSimple(m_cmdBufferCompute, nullptr, workgroup_count);
}

void SimpleCompute::Run()
{
