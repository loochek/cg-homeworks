...
layout (binding = 1) uniform sampler2D gFragCoord;
layout (binding = 2) uniform sampler2D gFragNormal;
layout (binding = 3) uniform sampler2D shadowMap;

void main()
{
  vec2 uv = gl_FragCoord.xy / vec2(Params.outputBufferWidth, Params.outputBufferHeight);
  vec3 wPos = texture(gFragCoord, uv).rgb;
  vec3 wNorm = texture(gFragNormal, uv).rgb;
